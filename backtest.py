'commitment of traders strategy, \
 ...
 realtime data: http://www.cftc.gov/dea/futures/financial_lf.htm, 3:30pm Friday '
...
# ======================== POST-PROCESSING, SPY ============================
# ------------------------    classical group   ----------------------------
# RSI value, RSI,    not statistically significant
w_RSI = 0.
very_high = SIGNAL[0]['RSI'] >= 80.
high      = (SIGNAL[0]['RSI'] >= 75.) & (SIGNAL[0]['RSI'] < 80.)
low       = (SIGNAL[0]['RSI'] <= 25.) & (SIGNAL[0]['RSI'] > 20.)
very_low  = SIGNAL[0]['RSI'] <= 20.
very_high *= w_RSI * -1.
high      *= w_RSI * -0.5
low       *= w_RSI * 0.5
very_low  *= w_RSI * 1.
RSI = very_high + high + low + very_low

# MFI value, MFIV, sell money flow is harder to exhaust than buy,
# use caution when buying under very negative MFI
w_MFI = 0.
very_high = SIGNAL[0]['MFI'] >= 80.
high      = (SIGNAL[0]['MFI'] >= 75.) & (SIGNAL[0]['MFI'] < 80.)
low       = (SIGNAL[0]['MFI'] <= 25.) & (SIGNAL[0]['MFI'] > 20.)
very_low  = SIGNAL[0]['MFI'] <= 20.
very_high *= w_MFI * -1.
high      *= w_MFI * -0.5
low       *= w_MFI * 0.5
very_low  *= w_MFI * 0.5
MFI = very_high + high + low + very_low

# momentum value, MOMV
# its performance deteriorites significantly from 2011 to 2015
w_MOM = 0.
trend = SIGNAL[0]['MOM']
very_high = (trend > 5.) * 1.
high = ((trend < 5.) & (trend > 2.) ) * 1.
low = ( (trend < -3.) & (trend > -6.) ) * 1.
very_low = (trend < -6.) * 1.
MOM = (-.5*very_high + high*.5 + low*-.5 + 5.*very_low) * w_MOM
...
# ===========================  score committee ==============================
INDICATOR = RSI + MFI + MOM + DOW + SP
SIGNAL[0]['INDC'] = INDICATOR

# backtest
future = SIGNAL[0].shift(periods=-1)    # holding for one week
dprice = future['CLOSE'] - SIGNAL[0]['CLOSE']
buy     = (SIGNAL[0]['INDC'] >=  1.)*1.
sell    = (SIGNAL[0]['INDC'] <= -1.)*1.
profit = (buy*1. - sell*1.)*dprice
history = pd.concat([buy-sell,profit],axis=1)

#profit = profit[profit!=0][:-1]
print('Sharpe:       ', profit[profit!=0].mean()/profit[profit!=0].std())
print('Sortino:      ', profit[profit!=0].mean()/profit[profit<0].std()/2.)
print('Drawdown:     ', profit[profit!=0].min())
print('Win rate:     ', sum(profit>0)*1./sum(profit!=0))
print('Sum:          ', profit.sum())
print('Abs Sum:      ', profit.abs().sum())
print('Num buys:     ', sum(buy))
print('Num sells:    ', sum(sell))
print('Active ratio: ', 1.*(sum(buy)+sum(sell))/len(profit))

# visualization
SPY_profit = (SIGNAL[0]['CLOSE'] / SIGNAL[0]['CLOSE'][0]).shift(-1)
COT_profit = 1. + profit.cumsum()/SIGNAL[0]['CLOSE'][0]
ax1, = plt.plot(SPY_profit,'blue')
ax2, = plt.plot(COT_profit,'red')
ax3, = plt.plot(SIGNAL[0]['INDC']/3.+1,'green')
#plt.legend([ax1,ax2],['S&P500 index','COT strategy'], loc=0)
